import { formatProspectListLabel } from "./prospect-provider.js";
import type {
  ProspectAgent,
  ProspectContact,
  ProspectList,
  ProspectListContactsResult,
  ProspectProvider,
} from "./prospect-provider.js";
...
/** Deterministic PRNG so mock data stays stable across server restarts. */
...
export class MockProspectProvider implements ProspectProvider {
  async listAgents(): Promise<ProspectAgent[]> {
    return MOCK_AGENTS;
  }

  async listProspectLists(agentId: string): Promise<ProspectList[]> {
  ...
  async listProspectListContacts(listId: string): Promise<ProspectListContactsResult> {
